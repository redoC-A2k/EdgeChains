...
// impl wit::exports::arakoo::demo::guest::Guest for Guest {
//     fn get_data() -> u8 {
//         105
//     }
// }
...
pub extern "C" fn init() {
    ...
    match res {
        Ok(_) => (),
        Err(err) => println!("Error : no input file specified or corrupted input js file supplied \n{}", err),
    }
    // println!("Contents : {}",contents);
    let runtime = runtime::new_runtime().unwrap();
    let context = runtime.context();
    match context.eval_global("function.js", &contents) {
        Ok(_) => (),
